...
export class OaktrackerComponent {
  ...
  @Input() hasPostgame: boolean = true;
  ...
  ngOnInit() {
    const locationTasks = [this.retrieveLocations("main", x => this.locationsMain = x)];
    if (this.hasPostgame) {
      locationTasks.push(this.retrieveLocations("postgame", x => this.locationsPostgame = x));
    }

    zip(locationTasks).subscribe(_ => this.retrieveAndAddHiddenGrottos().subscribe(_ => {
      this.initCaught();
      this.loaded = true;
    }));
  ...
  private retrieveLocations(set: string, setter: ((x: TrackerLocation[]) => void)): Observable<TrackerLocation[]> {
    return this.http.get(`assets/data/oaktracker/${this.folderName}/${set}.json`).pipe(
  ...
  private retrieveAndAddHiddenGrottos(): Observable<void> {
    return this.http.get(`assets/data/oaktracker/${this.folderName}/hidden_grotto.json`).pipe(
      map(r => {
